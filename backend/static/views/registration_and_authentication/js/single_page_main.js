...
function fillAlbumAboutBlock(groupId) {
	...
	fetch("http://localhost:3000/api/single" + `?single_id=${groupId}`, requestOptions)
		.then(async response => {
			...
			if (response.ok)
			{
				...
				fetch("http://localhost:3000/api/language" + `?id=${resultGroup.language_id}`, requestOptions)
					.then(async response => {
...
function onSinglePageLoad(afterWhichDivId) {

	tracksId = [];

	var urlParams = new URLSearchParams(window.location.search);
	var currPlaylistId = urlParams.get('single_id');
	...
	fetch("http://localhost:3000/api/single" + `?single_id=${currPlaylistId}`, requestOptions)
		.then(async response => {
					...
					div_post_content_block.className = "body-post-content-block";
						...
						div_post_content_main_block.className = "body-post-content-main-block";
							...
							div_playlist_block.insertAdjacentElement('beforeend', a_playlist_name_link);
									...
									tracksId.push(currPlaylist.track_id);
									fillTrackBlock(div_playlist_block, postNumber, 1, currPlaylist.track_id);
