...
function onGuestPageLoad(afterWhichDivId, userPlayerId) {
	...
	fetch("http://localhost:3000/api/user/", requestOptions)
		.then(async response => {
			var resultUsers = await response.json();
			if (response.ok)
			{
				for (item of resultUsers) {
					loadAllUserPosts(item.id, afterWhichDivId);
				}
			}				
			else
			{
				alert(resultUsers.message);
			}
		})
