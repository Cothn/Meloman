...
function fillAlbumAboutBlock(groupId) {
	...
	fetch("http://localhost:3000/api/album" + `?album_id=${groupId}`, requestOptions)
		.then(async response => {
			...
			if (response.ok)
			{
				...
				//languages
				fetch("http://localhost:3000/api/album/languages/" + `${groupId}`, requestOptions)
					.then(async response => {
...
//genres
				fetch("http://localhost:3000/api/album/genres/" + `${groupId}`, requestOptions)
					.then(async response => {
...
//labels
				...
				fetch("http://localhost:3000/api/label" + `?id=${resultGroup.label_id}`, requestOptions)
					.then(async response => {
				...
				var birthDate = new Date(resultGroup.release_date);
				document.getElementById(USER_ABOUT_DATE_LINK_ID).innerHTML +=
					"Release date: "+ birthDate.getDate()+"."+ birthDate.getMonth()+"."
...
function onAlbumPageLoad(afterWhichDivId) {

	tracksId = [];

	var urlParams = new URLSearchParams(window.location.search);
	var currPlaylistId = urlParams.get('album_id');
	...
	fetch("http://localhost:3000/api/album" + `?album_id=${currPlaylistId}`, requestOptions)
		.then(async response => {
			var resultPlaylist = await response.json();
			if (response.ok)
			{
				fillAlbumAboutBlock(currPlaylistId);
...
function makePlaylistBlock(postNumber, currPlaylist, afterWhichDivId) {
	...
	fetch("http://localhost:3000/api/album" + `/tracks/${currPlaylist.id}`, requestOptions)
		.then(async response => {
				...
				div_post_block.className = "body-post-block";
					...
					div_post_content_block.className = "body-post-content-block";
						...
						div_post_content_main_block.className = "body-post-content-main-block";
							...
							div_playlist_block.setAttribute("data-playlistTitle", "");
